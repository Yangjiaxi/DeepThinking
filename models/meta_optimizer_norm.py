...
class MomentumOptim:
    def __init__(self, step_size=0.01, momentum=0.0):
        self.step_size = step_size
        self.momentum = 0.0
        self.m = None  # velocity / gradient
...
class AttnOptimWrapper:
    ...
    def step(self, ctx_ids):
        ...
        if not self.kv:
            self.kv = cur_kv
            self.kv_diff_norm = [[torch.norm(k, 2), torch.norm(v, 2)] for k, v in self.kv]
        else:
            ...
            self.kv_diff_norm = [
                [torch.norm(diff_k, 2), torch.norm(diff_v, 2)] for diff_k, diff_v in zip(self.optim_k.m, self.optim_v.m)
            ]
